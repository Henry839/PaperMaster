...
struct ReaderView: View {
    ...
    @State private var isAskAIPopoverVisible = false
    ...
    @State private var marginAnnotationGeometry: ReaderMarginAnnotationGeometry?
    @FocusState private var marginNoteFocusField: ReaderMarginFocusField?
    ...
    var body: some View {
        ...
        .onChange(of: marginNoteFocusField) { oldValue, newValue in
    ...
    private var readerToolbar: some View {
        HStack(spacing: 8) {
            ...
            // Right group: search, display, appearance
            HStack(spacing: 8) {
                Button {
        ...
        ZStack(alignment: .topTrailing) {
            ZStack {
                ReaderPDFView(
                    ...
                    marginAnnotationGeometry: $marginAnnotationGeometry,
            ...
            if let geometry = marginAnnotationGeometry {
                ReaderMarginAnnotationOverlayView(
                    geometry: geometry,
                    annotations: sortedAnnotations,
                    appearanceMode: selectedAppearanceMode,
                    expandedCardID: $expandedAnnotationID,
                    spotlightedCardID: $spotlightedAnnotationID,
                    onJumpToAnnotation: { id in sendPDFCommand(.jumpToAnnotation(id)) },
                    ...
                    onUpdateColor: { annotation, color in
                        services.updateAnnotationColor(annotation, color: color, context: modelContext)
                        lastHighlightColorRawValue = color.rawValue
                    },
                    noteBindingProvider: { annotation in annotationNoteBinding(for: annotation) },
                    focusedField: $marginNoteFocusField,
                    onNoteFocusChanged: { id in
                        if id != nil {
                            scheduleSave()
                        }
                    }
            ...
            Color.clear
                ...
                .padding(.top, 60)
                .padding(.trailing, 18)
                .popover(isPresented: $isAskAIPopoverVisible) {
                    askAIPopover
                }
        }
    ...
    private var askAIPopover: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                Text("Ask AI")
                    .font(.headline)
            ...
            if let draft = askAISession.draft {
                ...
                TextField("Question", text: askAIQuestionBinding, axis: .vertical)
                    .textFieldStyle(.plain)
                    .lineLimit(2...5)
                    .padding(8)
                ...
                if let exchangeID = askAISession.lastExchangeID,
                   let lastExchange = askAISession.exchanges.first(where: { $0.id == exchangeID }) {
                    Divider()
                    ...
                    ScrollView {
                        VStack(alignment: .leading, spacing: 4) {
                            Text("Answer")
                                .font(.caption.weight(.semibold))
                                .foregroundStyle(.secondary)
                            ReaderMarkdownView(markdown: lastExchange.answer)
                        }
                        .frame(maxWidth: .infinity, alignment: .leading)
                    }
                    .frame(maxHeight: 300)
    ...
    private func addHighlight(from selection: ReaderSelectionSnapshot, openNoteEditor: Bool) {
        ...
        if openNoteEditor {
            ...
            DispatchQueue.main.async {
                marginNoteFocusField = .marginNote(annotation.id)
            }
...
private struct ReaderPDFView: NSViewRepresentable {
    ...
    @Binding var marginAnnotationGeometry: ReaderMarginAnnotationGeometry?
    let expandedAnnotationID: UUID?
    ...
    func makeCoordinator() -> Coordinator {
        Coordinator(
            ...
            marginAnnotationGeometryBinding: $marginAnnotationGeometry
        )
    }
    ...
    func updateNSView(_ nsView: InteractivePDFView, context: Context) {
        ...
        context.coordinator.marginAnnotationGeometryBinding = $marginAnnotationGeometry
        context.coordinator.expandedAnnotationID = expandedAnnotationID
    ...
    final class Coordinator: NSObject {
        ...
        var marginAnnotationGeometryBinding: Binding<ReaderMarginAnnotationGeometry?>
        var expandedAnnotationID: UUID?
        var currentAnnotations: [PaperAnnotation] = []
        ...
        init(
            ...
            marginAnnotationGeometryBinding: Binding<ReaderMarginAnnotationGeometry?>
        ) {
            ...
            self.marginAnnotationGeometryBinding = marginAnnotationGeometryBinding
        }
        ...
        func updateOverlayGeometry(in pdfView: PDFView) {
            ...
            updateMarginAnnotationGeometry(in: pdfView)
        }

        func updateMarginAnnotationGeometry(in pdfView: PDFView) {
            let expandedID = expandedAnnotationID
            let annotatedOnly = currentAnnotations.filter { !$0.noteText.isEmpty || $0.id == expandedID }
            marginAnnotationGeometryBinding.wrappedValue = ReaderMarginAnnotationGeometry.capture(
                annotations: annotatedOnly,
                in: pdfView
            )
        }
